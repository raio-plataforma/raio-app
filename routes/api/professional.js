            ...
            .then(professional => {
                if (professional) {
                    ...
                    const newProfessional = new Professional({
                        ...
                        cnpj: req.body.cnpj,
                        cnpj_type: req.body.cnpj_number,
                        cnpj_number: req.body.cnpj_number,
...
router.get('/all', (req, res) => {
    const errors = {}
    Professional.find().populate('user_id')
        .sort({ createdAt: -1 })
        .then(professionals => {
            if (!professionals) {
                errors.professionals = 'Não existem profissionais cadastradas ainda'
                return res.status(404).json(errors)
            }
            res.json(professionals)
        })
        .catch(() => res.status(404).json({
            professionals: 'Não existem profissionais cadastradas ainda'
        }))
})
